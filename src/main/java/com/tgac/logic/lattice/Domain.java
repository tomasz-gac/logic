...
// ABOUTME: value carrying its own capability record â membership, collapse, stabilization.
...
/**
 * What a {@link LatticeStore} requires of its per-name values â the capability
...
public interface Domain<L extends Domain<L>> extends Semilattice<L>, PartialOrder<L>, Absorbing {
	...
	/**
	 * The single value this domain has collapsed to, if any â the store turns
	 ...
	 * licensed only by strict narrowing, so do not weaken it. The default â
	 * exact equality â is the finite-descent policy; a lattice with infinite
	 * descending chains (reals) MUST override with its Îµ/widening policy, a
