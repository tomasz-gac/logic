...
public class ImpositionLawsTest {
	...
	private static final class World {
		...
		Package state(int impositions) {
			...
			for (int i = 0; i < impositions; i++) {
				List<Package> worlds = Trial.imposed(literal(), p).get();
				if (worlds.size() == 1) {
					p = worlds.head();
				}
			}
	...
	public void impositionIsIdempotent() {
		...
		for (long seed = 0; seed < SEEDS; seed++) {
			...
			List<Package> once = Trial.imposed(literal, p).get();
			if (once.size() != 1) {
				continue;
			}
			exercised++;
			List<Package> twice = Trial.imposed(literal, once.head()).get();
	...
	public void quiescentNormalizeIsAFixpoint() {
		...
		for (long seed = 0; seed < SEEDS; seed++) {
			...
			for (Object store : p.getStores().values()) {
				if (!(store instanceof Absorbable)) {
					continue;
				}
				exercised++;
				Absorbable<?> cs = (Absorbable<?>) store;
				final long s = seed;
				cs.normalize(p).get().match(
						() -> {
	...
	public void noSilentSwallowing() {
		...
		for (long seed = 0; seed < SEEDS; seed++) {
			...
			List<Package> bound = Trial.imposed(Posting.bind(x, lval(9L)), p).get();
	...
	public void groundImpositionDecides() {
		...
		for (long seed = 0; seed < SEEDS; seed++) {
			...
			exercised++;
			List<Package> worlds = Trial.imposed(ground, Package.empty()).get();
			if (worlds.isEmpty()) {
				continue;
			}
