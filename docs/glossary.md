...
- [~] **unit propagation** ⋯import — a disjunction with every alternative refuted but one imposes that one: choice decided by elimination, cascading through revise; zero survivors fails the branch (DPLL's core rule, 1962; two watched literals its efficient form, Chaff 2001). *(either-store-pays-in-products note)*
