...
- [x] **Factor** — the execution face of a store: two triggers (delta and focused normalize) each handed the pair's theory, plus enforce and reify, answered by `Fiber<Revision>`; never holds the knowledge, may keep private MEMO state (reconstructible from the theory — droppable), may read anything, may swap only its OWN entry (factor included); carries no algebra of its own. Retires *ConstraintStore*, *Absorbable*, *Projectable* (their capabilities live on Theory) and *stated* (a statement is the door's meet plus a Met). *(constraint-kernel §3)*
- [x] **normalize(Prefix, ·)** — bindings arrived; the store's COMPLETE delta reaction (custody, own watchers walked at examination, own cascade). Retires *revise* as vocabulary. *(constraint-kernel)*
- [x] **absorb** — the wholesale door: meet an incoming THEORY into the resident store (seeding from the atoms' own empty when absent, skipping under the covering guard) + queue a Met of the arrived atoms. *(constraint-kernel §2)*
- [x] **Met** — the agenda row knowledge doors queue: (family, the atoms that ARRIVED — a statement's atom, an absorbed theory's); the focus is caller-known arithmetic, never a computed diff. Retires the *Stated* and *Absorbed* rows. *(constraint-kernel §2)*
- [x] **normalize(focus, ·)** — the focused trigger: re-establish normal form from the arrived atoms, under the ONE LAW `normalize(T, F, P) ≡ normalize(T, T.atoms(), P)` whenever the focus contains the true change — skipping only what the focus cannot have touched is licensed, doing more is always sound. Retires *normalize (store)*, the wholesale trigger. *(constraint-kernel §3)*
- [x] **Verifier** — the marker of a trial-based family: its crossed-off reading is package equality, so it presupposes a base where every value family finished reacting — honored structurally by folding marked families LAST; the trial base is `Propagation.scratch`. *(constraint-kernel §3)*
...
- [x] **Doomed** — the refutation capability: a cheap own-semantics born-violated check, monotone under binding growth; read by the activation door, consumed by the doom pruning pass, absent claims nothing. *(constraints/store)*
- [x] **Constraint (pair)** — SHIPPED (#137, Aug 2026): the package's constraint entry as Constraint{Theory, Factor} — knowledge outside, behavior and memo beside it; identity is the theory ALONE (factor state = reconstructible memo, marshal never carries it); Revision returns the pair. *(constraint-kernel §3)*
...
- [x] **Name** — the one identity type (ratified Aug 2026, retiring the *Unknown* placeholder): an identity that stands for a value without being one; `LVar` is a name in state position (epistemic, undetermined), `Hole`/*Any* a name in answer position (ontic — certified unconstrained, denotes every value). *(unknown-name-type note)*
- [~] **unit propagation** ⋯import — a disjunction with every alternative refuted but one imposes that one: choice decided by elimination, cascading through revise; zero survivors fails the branch (DPLL's core rule, 1962; two watched literals its efficient form, Chaff 2001). *(disjunction-store-pays-in-products note)*
